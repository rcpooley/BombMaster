...
for (let numWires = 3; numWires <= 6; numWires++) {
    const conds = CONDITIONS[numWires];
    all[numWires] = {};
    for (let i = 0; i < conds.length; i++) {
        all[numWires][i] = [];
        for (let j = 0; j < 1000; j++) {
            const constraints = Gen.getConstraints(numWires, conds, i);
            const fil = all[numWires][i].filter(o =>
                Object.compare(o, constraints)
            );
            if (fil.length === 0) {
                all[numWires][i].push(constraints);
            }
        }
    }
}
...
for (let numWires = 3; numWires <= 6; numWires++) {
    const allNW = all[numWires];
    const keys = Object.keys(allNW);
    for (let i = 1; i < keys.length; i++) {
        const allI = allNW[keys[i]];
        for (let j = 0; j < allI.length; j++) {
            const constraints = allI[j];
            let minWires = 0;
            let maxWires = numWires * COLORS.length;
            ...
            COLORS.forEach(c => {
                ...
                if ('max' in cc) {
                    maxWires -= numWires - cc.max;
                }
            });

            if (minWires > numWires) {
                console.log('MIN', numWires, i, constraints);
            }
            if (maxWires < numWires) {
                console.log('MAX', numWires, i, constraints);
            }

            console.log(numWires, i, j);
            for (let k = 0; k < 1000; k++) {
                const config = Gen.getConfigForConstraints(
                    constraints,
                    numWires
                );
                if (
                    config.wires.length !== numWires ||
                    config.wires[-1] !== undefined
                ) {
                    console.log('BADWIRES', numWires, i, config);
                    process.exit();
                }
                const idx = Gen.getConditionIdx(config, CONDITIONS[numWires]);
                if (idx !== i) {
                    console.log('MISMATCH', numWires, `e${i} a${idx}`, config);
                    process.exit();
                }
            }
        }
